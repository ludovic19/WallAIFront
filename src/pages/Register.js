// évite le rechargement par défaut de la page
...
const Register = () => {
  ...
  const createUser = (e) => {
    e.preventDefault();
    // objet multer multer
    const newUser = new FormData(); // créer un objet avec name,email, password
    ...
    const register = (abc) => {
      return axios
        ...
        .catch((err) =>
          setError(
            "Email already used. Please log in or register with an other email."
          )
        );
    };
    register();
  ...
  return (
    <>
      <div className="w-full flex justify-between items-center bg-white sm:px-8 px-4 py-4 border-b border-b-[#e6ebf4]">
        <Link
          to="/"
          className="font-inter front-medium bg-[#6469ff] text-white px-4 py-2 rounded-md"
        >
          Back
        </Link>
        <Link
          to="/login"
          className="font-inter front-medium bg-green-600 text-white px-4 py-2 rounded-md"
        >
          Login
        </Link>
      </div>
          ...
          <form noValidate onSubmit={createUser}>
            <label>
              ...
              <div className="text-center">
                <button
                  className="mt-4 text-white bg-[#6469ff] font-medium rounded-md text-xl w-full sm:w-auto px-5 py-2.5 text-center"
                  type="submit"
                >
                  Register
                </button>
      ...
      <footer className="w-full flex justify-between items-center mt-64 sm:px-8 px-4 py-4 border-t border-t-[#e6ebf4]">
        ...
        <h2 className="font-bold max-sm:hidden">
          Created By :<br />{" "}
          <span className="font-bold text-[#6449ff] text-xl">
            Hicham HAJLA
            <br /> & Ludovic LEVEQUE
          </span>
        </h2>
